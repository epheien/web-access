...
// --- 通过 HTTP API 获取最新 WebSocket URL（本地特性，合并保留） ---
...
async function discoverChromePort() {
  ...
  if (result.kind === 'ok') {
    ...
    // 通过 HTTP API 刷新最新 WebSocket URL，解决浏览器重启后旧 WS 路径失效的问题（本地特性保留）
    const freshWsPath = await fetchChromeWebSocketUrl(result.browser.port);
    const wsPath = freshWsPath || result.browser.wsPath;
    const tag = result.source === 'override' ? '[--browser 指定]' : '[config.env 偏好]';
    console.log(`[CDP Proxy] 选用 ${result.browser.label} (端口 ${result.browser.port}${wsPath ? '，带 wsPath' : ''}) ${tag}`);
    return { port: result.browser.port, wsPath };
  }
  ...
  if (fallbackPort !== null) {
    ...
    // 手动端口兜底同样用 HTTP API 获取最新 WS URL（本地特性保留）
    const freshWsPath = await fetchChromeWebSocketUrl(fallbackPort);
    console.log(`[CDP Proxy] 通过手动调试端口连接: ${fallbackPort}`);
    return { port: fallbackPort, wsPath: freshWsPath };
  }
  return null;
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // GET /screenshot?target=xxx&file=/tmp/x.png - 截图（默认整页）
    else if (pathname === '/screenshot') {
      const sid = await ensureSession(q.target);
      const format = q.format || 'png';
      // 未传 fullPage 或 fullPage != '0' 时默认截取整页
      const captureBeyondViewport = !q.fullPage || q.fullPage !== '0';
    ...
    // GET /info?target=xxx - 获取页面信息
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          ...
          '/screenshot?target=&file=&fullPage=0': 'GET - 截图（默认整页，fullPage=0 仅截可视区域）',
        },
      }));
    }
  } catch (e) {
